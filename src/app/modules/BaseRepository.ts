...
export class BaseRepository<T extends Document> implements IWrite<T>, IRead<T> {
    ...
    counterBR = async (counter: any): Promise<{}> => {   //Todo can be improved more by remaining key and value pair replace it directly with data.
        let count: any = {}
        //@ts-expect-error
        if(!counter.length) await this._model.countDocuments({ isDeleted: false }).then((value: Number) => count['total'] = value)
        let countMap = counter.map(async (counter : any) => {
            //@ts-expect-error
