...
export class OrderRepository extends BaseRepository<IOrder> {
    ...
    create = async (newData: any, session: ClientSession): Promise<IOrder|never> => {
        
        return await orderModel.create([newData], {session}).then(user => user[0])
    }

    findById = async (_id: string): Promise<IOrder|null> => {
        const aggregate = [
        ...
        return await orderModel.aggregate<IOrder>(aggregate).then(user => user[0] || null)
    }
