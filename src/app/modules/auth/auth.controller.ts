...
// import {ISession, sessionLoginTypeEnum} from "../session/session.types";
...
export class AuthController  {
    ...
    //         const verifyToBeInserted = {userId: userData._id, operation: 'login', otp, ip, createdBy: userData._id, updatedBy: userData._id}
    //         //@ts-expect-error
    //         const verifyData = await verificationBusinessInstance.createBB(verifyToBeInserted)
    //         if(verifyData)
    //         {
    ...
    //                 const jwt_token_encrypt = await jwt.sign({_id: userData._id, email: userData.email, companyId: userData.companyId,firstName:userData.firstName,lastName:userData.lastName, roleId: roleData?._id, roleName: roleData?.shortDescription}, Constant.jwt_key, {expiresIn: 28800})
    ...
    async devLogin(req: Request, res: Response): Promise<void> {
        try
        {
            ...
            let userData = await userModel.findOne({email: body.email, isDeleted: false, isActive: true})
            // const [roleData] = await Promise.all([await new RoleRepository().findOneBR({_id: userData?.roleId}),
                // await new CompanyRepository().findOneBR({_id: userData?.companyId})]);
            if(!userData) {
                res.locals.data = null;
                res.locals.message = "Invalid Email";
                ...
                // req.body.loggedInUser.roleName = roleData?.shortDescription;
                // const permissionData = await new PermissionController().per(req, res);
                const user: object = {_id: userData._id, email: userData.email, firstName: userData.firstName, lastName: userData.lastName};
                const jwt_token_encrypt = await jwt.sign({_id: userData._id, email: userData.email,firstName:userData.firstName,lastName:userData.lastName}, Constant.jwt_key, {expiresIn: 28800});
                const jwt_token = await AES.encrypt(jwt_token_encrypt, Constant.secret_key).toString();
                // await sessionModel.create({userId: userData._id, ip, token: jwt_token, loginType: sessionLoginTypeEnum.email})
